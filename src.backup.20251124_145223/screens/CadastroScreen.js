    ...
    try {
      ...
      Alert.alert(
        ...
        [
          ...
          {
            text: 'Continuar (Modo Teste)',
            onPress: async () => {
              // Salva dados fictícios no AsyncStorage para modo teste
              await AsyncStorage.setItem('@ColheCash:token', 'test-token');
              await AsyncStorage.setItem('@ColheCash:user', JSON.stringify({ 
                id: '1', 
                nome, 
                email,
                telefone
              ...
              // Navega para a Home em modo teste
              navigation.reset({
                index: 0,
                routes: [{ name: 'Home' }],
              });
