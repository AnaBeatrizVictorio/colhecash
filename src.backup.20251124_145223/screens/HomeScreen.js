...
export default function HomeScreen() {
  ...
  const [isMetaInputVisible, setIsMetaInputVisible] = useState(false);
  const [planejamentoMeta, setPlanejamentoMeta] = useState(0);
  const [planejamentoDespesa, setPlanejamentoDespesa] = useState(0);
  const [planejamentoReceita, setPlanejamentoReceita] = useState(0);
  ...
  const fetchData = async () => {
    try {
      ...
      // ⚠️ CORRIJA ESTA LINHA:
      const responseConfiguracoes = await api.get("/api/configuracoes/1");
      const metaSalva = responseConfiguracoes.data.metaFaturamento || 0;

      const porcentagemMeta =
        metaSalva > 0 ? Math.round((totalVendas / metaSalva) * 100) : 0;
      const porcentagemDespesa =
        totalVendas > 0 ? Math.round((totalDespesas / totalVendas) * 100) : 0;
      const porcentagemLucro =
        totalVendas > 0 ? Math.round((lucro / totalVendas) * 100) : 0;
      ...
      setPlanejamentoTexto({
        meta: `R$ ${totalVendas.toFixed(2).replace(".", ",")} de R$ ${metaSalva
          .toFixed(2)
          .replace(".", ",")} (${Math.min(porcentagemMeta, 100)}% da meta)`,
        despesas: `R$ ${totalDespesas.toFixed(2).replace(".", ",")} (${Math.min(
          porcentagemDespesa,
          100
        )}% do faturamento)`,
        lucro: `R$ ${lucro.toFixed(2).replace(".", ",")} (${Math.min(
          porcentagemLucro,
          100
        )}% do faturamento)`,
      });
  ...
  const handleSaveMetaFaturamento = async () => {
    setSavingMeta(true);
    try {
      const metaParaSalvar = parseFloat(metaFaturamentoInput.replace(",", "."));
      
      if (isNaN(metaParaSalvar) || metaParaSalvar <= 0) {
        Alert.alert("⚠️ Erro", "Por favor, insira um valor válido maior que zero.");
        setSavingMeta(false);
        return;
      }

      console.log("📤 Salvando meta:", metaParaSalvar);

      // ⚠️ CORRIJA ESTA LINHA:
      const response = await api.put("/api/configuracoes/1", { 
        metaFaturamento: metaParaSalvar 
      });

      console.log("✅ Resposta do servidor:", response.data);

      const metaSalva = metaParaSalvar.toFixed(2).replace(".", ",");
      setMetaFaturamento(`R$ ${metaSalva}`);
      
      Alert.alert("✅ Sucesso", "Meta de faturamento salva com sucesso!");
      setIsMetaInputVisible(false);
      ...
      Alert.alert(
        "❌ Erro", 
        error.response?.data?.message || "Erro ao salvar meta de faturamento. Tente novamente."
      );
    } finally {
      setSavingMeta(false);
    }
    ...
    <>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.saldosRow}>
            <View style={styles.saldoCol}>
              <Text style={styles.saldoValorVerde}>
                {loadingData ? "Carregando..." : lucroTotal}
              </Text>
              <Text style={styles.saldoLabel}>Lucro Total</Text>
            </View>
            <TouchableOpacity
              style={styles.saldoCol}
              onPress={() => setIsMetaInputVisible(!isMetaInputVisible)}
            >
              <Text style={styles.saldoValorAmarelo}>
                {loadingData ? "Carregando..." : metaFaturamento}
              </Text>
          ...
          <View style={styles.cardsRow}>
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate("ResumoFinanceiro", { filter: "despesas" })
              }
            >
              ...
              <View style={styles.cardContent}>
                {loadingData ? (
                  ...
                  <Text style={styles.cardItemDespesa}>
            ...
            <TouchableOpacity
              style={styles.cardSmall}
              onPress={() =>
                navigation.navigate("ResumoFinanceiro", { filter: "receitas" })
              }
            >
              <View style={styles.cardHeader}>
                <View style={styles.cardTitleContainer}>
              ...
              <View style={styles.cardContent}>
                {loadingData ? (
                  ...
                  <Text style={styles.cardItemReceita}>
          ...
          {/* Card Calculadora */}
          <TouchableOpacity
            style={[styles.cardSmall, { width: "95%", marginBottom: 20 }]}
            onPress={() => navigation.navigate("CalculadoraPreco")}
          >
            <View style={styles.cardHeader}>
              <View style={styles.cardTitleContainer}>
                <Ionicons
                  ...
                  style={{ marginRight: 5 }}
                />
                <Text style={[styles.cardTitleReceita, { color: "#00ff00" }]}>
                ...
                <View
                  style={[
                    styles.barraMeta,
                    {
                      width: loadingData
                        ? "0%"
                        : `${Math.min(planejamentoMeta, 100)}%`,
                    },
                  ]}
                />
              </View>
              <Text style={styles.porcentagem}>
                {loadingData ? "--%" : `${Math.min(planejamentoMeta, 100)}%`}
              </Text>
            </View>
            <View style={styles.planejamentoLinha}>
                ...
                <View
                  style={[
                    styles.barraDespesa,
                    {
                      width: loadingData
                        ? "0%"
                        : `${Math.min(planejamentoDespesa, 100)}%`,
                    },
                  ]}
                />
              </View>
              <Text style={styles.porcentagem}>
                {loadingData ? "--%" : `${Math.min(planejamentoDespesa, 100)}%`}
              </Text>
            </View>
            <View style={styles.planejamentoLinha}>
                ...
                <View
                  style={[
                    styles.barraReceita,
                    {
                      width: loadingData
                        ? "0%"
                        : `${Math.min(planejamentoReceita, 100)}%`,
                    },
                  ]}
                />
              </View>
              <Text style={styles.porcentagem}>
                {loadingData ? "--%" : `${Math.min(planejamentoReceita, 100)}%`}
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.menuContainer}>
          ...
          <ScrollView style={styles.menuList}>
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("ResumoFinanceiro");
              }}
            >
              <Ionicons
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("RegistrarVenda");
              }}
            >
              <Ionicons
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("RegistrarDespesa");
              }}
            >
              <Ionicons
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("ControleDeGastos");
              }}
            >
              <FontAwesome5
                name="dollar-sign"
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("DicasFinanceiras");
              }}
            >
              <Ionicons
                name="bulb-outline"
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("AssistenteIA");
              }}
            >
              <Ionicons
                name="bulb"
                ...
                style={styles.menuIcon}
              />
              <Text style={[styles.menuText, { color: "#FFD700" }]}>
                ASSISTENTE IA 🤖
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("AlertasENotificacoes");
              }}
            >
              <Ionicons
                name="notifications-outline"
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setMenuVisible(false);
                navigation.navigate("TermosECondicoes");
              }}
            >
              <Ionicons
                name="document-text-outline"
      ...
      >
        ...
        >
          ...
          >
            {Array.from({ length: 101 }, (_, i) => 2000 + i).map((ano) => (
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() => setFabOptionsVisible(true)}
      >
