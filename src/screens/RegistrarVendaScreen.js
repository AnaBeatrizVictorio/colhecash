...
import { View, Text, TouchableOpacity, StyleSheet, TextInput, Modal, FlatList, ActivityIndicator, Alert } from 'react-native';
...
export default function RegistrarVendaScreen() {
  ...
  useEffect(() => {
    const fetchCategorias = async () => {
      try {
        ...
        // Simule uma chamada de API para buscar categorias de venda
  ...
  function handleTecla(tecla) {
    if (tecla === '=') return; // Não faz nada por enquanto
  ...
  const handleSaveVenda = async () => {
    if (!valor) {
      Alert.alert('Erro', 'Por favor, insira um valor para a venda.');
      return;
    }
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Falha ao registrar venda. Tente novamente.';
  ...
  return (
    <View style={styles.bg}>
      ...
      <View style={styles.headerAzul}>
        <Text style={styles.headerAzulText}>Registrar Venda</Text>
      </View>
      {/* Modal preto */}
      <View style={styles.modalPreto}>
        <View style={styles.modalHeader}>
          ...
          <TouchableOpacity onPress={handleSaveVenda} disabled={saving}>
            {saving ? (
